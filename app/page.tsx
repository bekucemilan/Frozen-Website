...
import { supabase, Product, SiteInfo, Social, AboutSection, FeatureBadge } from '@/lib/supabase';
...
async function getData() {
  const [{ data: products }, { data: siteInfo }, { data: socials }, { data: sections }, { data: badges }] = await Promise.all([
...
export default async function Home() {
  const { products, siteInfo, socials, sections, badges } = await getData();
  ...
  const waLink = waSocial?.url
    ...
    : (siteInfo?.phone ? `https://wa.me/${siteInfo.phone.replace(/\D/g, '')}` : null);
  const heroBadges = badges.filter((b) => b.section === 'hero');
  const footerBadges = badges.filter((b) => b.section === 'footer');
  const extraSections = sections.filter((s) => s.title?.trim() && s.body?.trim());
  ...
  return (
    <>
      ...
      <section className="about">
        ...
        {extraSections.map((s) => (
          <div className="wrap about-grid about-extra" key={s.id}>
              ...
              <div className="socials-row">
                ...
                {socials.map((s) => (
                  <a
                    ...
                    title={SOCIAL_LABELS[(s.platform || '').trim().toLowerCase()] || s.platform}
                  >
